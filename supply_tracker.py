...
def apply_patient_day(state: InventoryState, patients: int, visit_date: str) -> None:
...
def cmd_log_day(args: argparse.Namespace) -> None:
    ...
    apply_patient_day(state, args.patients, visit_date)
    save_state(state, args.file)

    print(f"Logged {args.patients} IV ketamine patient(s) for {visit_date}.")
...
def rows_from_csv_text(text: str) -> Iterable[dict]:
    return csv.DictReader(io.StringIO(text))


def google_sheet_csv_url(sheet_url: str, gid: str | None) -> str:
    ...
    doc_id = parts[parts.index("d") + 1] if "d" in parts else None
    if not doc_id:
        raise ValueError("Could not find Google Sheet id")
...
def import_patient_rows(state: InventoryState, rows: Iterable[dict]) -> int:
    added = 0
    for row in rows:
        if not row:
            continue
        if "date" not in row or "patients" not in row:
            raise ValueError("CSV must include headers: date,patients")
        visit_date = parse_date(str(row["date"]).strip())
        patients = int(str(row["patients"]).strip())
        apply_patient_day(state, patients, visit_date)
        added += 1
    return added


def cmd_import_patients_csv(args: argparse.Namespace) -> None:
    state = load_state(args.file)
    ...
    added = import_patient_rows(state, rows)

    save_state(state, args.file)
    print(f"Imported {added} patient day row(s) from {args.csv_file}")


def cmd_import_patients_google(args: argparse.Namespace) -> None:
    state = load_state(args.file)
    if args.reset_history:
        state.history = []

    export_url = google_sheet_csv_url(args.sheet_url, args.gid)
    with urlopen(export_url) as response:  # nosec - user-provided URL for sheet import
    ...
    save_state(state, args.file)
    print(f"Imported {added} patient day row(s) from Google Sheet")
    print(f"CSV URL used: {export_url}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_import_csv = sub.add_parser(
        "import-patients-csv",
        help="Import patient history from CSV with headers: date,patients",
    )
    p_import_csv.add_argument("csv_file", help="CSV file path")
    p_import_csv.add_argument(
        "--reset-history",
        action="store_true",
        help="Clear existing history before import",
    )
    p_import_csv.set_defaults(func=cmd_import_patients_csv)

    p_import_google = sub.add_parser(
        "import-patients-google",
        help="Import patient history from a Google Sheet (requires shared access)",
    )
    p_import_google.add_argument("sheet_url", help="Google Sheet URL")
    p_import_google.add_argument(
        "--gid",
        help="Sheet tab gid (optional; defaults to gid in URL or 0)",
    )
    p_import_google.add_argument(
        "--reset-history",
        action="store_true",
    ...
    p_import_google.set_defaults(func=cmd_import_patients_google)
